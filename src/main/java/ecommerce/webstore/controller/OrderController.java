...
public class OrderController {
	...
	public String goToDashBoard(@ModelAttribute("processedOrder")ProcessedOrderDetails processedOrder, Principal user, HttpSession session, RedirectAttributes redirectAttr) {
		Member member = memberService.findByUserName(user.getName()).get(0);
		Order currentorder = (Order) session.getAttribute("currentorder");
	...
	public String checkout(@Valid @ModelAttribute("processedOrder")ProcessedOrderDetails processedOrder, 
			...
			SessionStatus status, Principal user, RedirectAttributes redirectAttr) {
	...
	public String trackOrderDetails(@RequestParam("id") long id, Model model){
